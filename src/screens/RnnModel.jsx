...
const RnnModel = () => {
  ...
  const prepareInput = (input) => {
    const cleanedText = input.replace(
      /[!"#$%&'()*+,--./:;<=>?@[\]^_`{|}~]/g,
      "",
    );
  ...
  const predict = async (inputText) => {
    ...
    const model = await tf.loadLayersModel(
      "https://fynix-dev.github.io/OpenModels/models/model_rnn/model.json",
    );
    //convert it into tensorflow tensor
    ...
    const predDict = [];
    //populate the prediction dictionary
    predDict.push(
      {
        label: labels[resArray[0].indexOf(Math.max(...resArray[0]))],
        value: Math.max(...resArray[0]).toFixed(3) * 100,
      },
      {
        label: labels[resArray[0].indexOf(Math.min(...resArray[0]))],
        value: Math.min(...resArray[0]).toFixed(3) * 100,
      },
    );
  ...
  return (
    <section className="w-full h-screen flex items-center justify-center bg-bg">
      <div className="w-fit min-w-[45%] md:max-w-[70%] container">
        <h1 className="h1">Movie Review Tone Predictor</h1>

        <div className="w-full h-max-[60px] h-[45px] flex items-center border-2 border-black rounded-lg">
          <input
            className="focus:outline-0 rounded-lg w-full h-full pl-[8px]"
            id="review-input"
            placeholder="Enter the Movie Review"
          ></input>
          <div
            onClick={() =>
              predict(document.getElementById("review-input").value)
            }
          ...
          <div className="w-full h-auto flex items-center justify-center flex-col gap-[4px]">
            {isLoading ? (
              ...
              prediction.map((item, index) => (
                <div
                  key={index}
                  className="nth-[1]:font-bold font-medium nth-[1]:text-black text-gray-800"
                >
