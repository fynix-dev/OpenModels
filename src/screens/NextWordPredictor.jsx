...
const NextWordPredictor = () => {
  ...
  const predictNextWord = async(inputText) => {
    ...
    const res = await fetch('https://technicalsquad-next-word-predictor.hf.space/predict', {
        method: "POST",
      ...
      <div className="w-full h-full min-h-screen bg-bg flex items-center justify-center p-[20px] md:p-[0px]">
        <div className="container">
          ...
          <div className='w-full h-max-[60px] h-[45px] flex items-center border-2 border-black rounded-[8px]'>
            <input className="focus:outline-0 rounded-[8px] w-full h-full pl-[8px]" id="review-input" placeholder='Enter the Sentence'></input>
            <div 
              onClick={() => predictNextWord(document.getElementById('review-input').value)}
